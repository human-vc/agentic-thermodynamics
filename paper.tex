...
\subsection{Experimental Validation Challenges}\label{sec:validation}

During extended validation with 144 trials, we initially observed near-perfect consensus across all conditions. Initial analysis suggested RLHF-induced homogenization, but careful debugging revealed the true cause: \textbf{API rate limiting}.

At high request volumes, GPT-4o-mini returns 429 errors. Our code fell back to identical default responses (``Based on available information, further analysis is needed''), artificially producing consensus scores of 1.000. This was not genuine convergence but an experimental artifact.

After implementing exponential backoff and increasing rate-limit delays, we reran trials and observed normal consensus dynamics (scores 0.25--0.40) with proper diversity preservation. The 12-trial clean results reported in Section~\ref{sec:results} represent valid data collected under proper rate-limiting conditions.

This experience highlights the importance of rigorous debugging in LLM-based experiments. Rate limiting, caching, and API errors can masquerade as model behavior, requiring careful instrumentation to distinguish genuine phenomena from experimental artifacts.
...
\begin{itemize}
    ...
    \item Rate limiting constraints limited our trial count; larger-scale validation needed
    \item API costs and latency prevent real-time multi-agent deployments
\end{itemize}
